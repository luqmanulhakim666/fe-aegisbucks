...
export default {
  ...
  methods: {
    ...
    getMediaName() {
      return this.form?.backgroundImage?.name || null;
    },

    onPreview() {
      let url = "";

      if (this.form?.backgroundImage?.url) {
        url = this.form?.backgroundImage?.url;
      }

      if (this.form?.backgroundImage?.id) {
        url = `${this.$config.API_URL}/file/${this.form?.backgroundImage?.id}/file`;
      }
