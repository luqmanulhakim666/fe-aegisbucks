<template>
  <div>
    <general-table-header
      label="Brand"
      actionCreate="/admin/banners/create"
      @on:sort="onSort"
      @on:search="onSearch"
      :exportReport="false"
      sort
    ...
    >
      ...
      <template v-slot:[`item.variants`]="{ item }">
        <ul>
          <li v-for="(variant, index) in item.variants" :key="index">
            <p class="text--small dark--text">{{ variant.name }}</p>
          </li>
        </ul>
      </template>
...
export default {
  ...
  methods: {
    ...
    async onUpdateStatus(val) {
      const res = await this.$api.banners.update(val.id, {
    ...
    async fetch() {
      ...
      const res = await this.$api.banners.getList({ ...this.body });
    ...
    async onDelete() {
      ...
      const res = await this.$api.banners.delete(id);
    ...
    onEdit(id) {
      this.$router.push(`/admin/banners/${id}`);
    },
  },
};
</script>
