<template>
  <div>
    <general-table-header
      label="Batch"
      actionCreate="/admin/assessment/batch/create"
      filterYear
      @on:filterYear="onFilterYear"
      @on:search="onSearch"
    />
    <v-data-table
      :headers="headers"
      :loading="state.isLoading"
      :items="items"
      hide-default-footer
      no-data-text="No Data"
      class="mt-8"
    >
      <template v-slot:[`item.action`]="{ item }">
        <v-menu auto offset-x rounded="xxl">
          <template v-slot:activator="{ on, attrs }">
            <span v-bind="attrs" v-on="on">
              <v-icon>mdi-dots-vertical</v-icon>
            </span>
          </template>
          <v-list>
            <v-list-item @click="onDetail(item.id)">
              <v-list-item-title class="d-flex align-center">
                <v-icon size="16" class="mr-3 dark--text"> mdi-eye </v-icon>
                <span class="text--small">Detail </span>
              </v-list-item-title>
            </v-list-item>

            <v-list-item @click="handleDialogDelete(item.id)">
              <v-list-item-title class="d-flex align-center">
                <v-icon size="16" class="mr-3 dark--text"> mdi-delete </v-icon>
                <span class="text--small dark--text">Hapus</span>
              </v-list-item-title>
            </v-list-item>
          </v-list>
        </v-menu>
      </template>
    </v-data-table>

    <general-pagination
      class="mt-6"
      :perPage="body.limit"
      :total="paging.count"
      :totalPage="paging.totalPage"
      :paging="paging"
      @on:change="onChangePagination"
    />

    <general-dialog-delete
      :dialog="state.dialogDelete"
      @on:close="handleDialogDelete"
      @on:delete="onDelete()"
    />
  </div>
</template>

<script>
import meta from '@/mixins/meta'
import alert from '@/mixins/alert'
import pipe from '@/mixins/pipe'
import routes from '@/mixins/routes'

import debounce from 'lodash/debounce'

export default {
  mixins: [meta, alert, pipe, routes],
  middleware: ['authenticated', 'authorized'],
  meta: {
    page: ['admin'],
    permission: ['super_admin']
  },
  data: () => ({
    meta: {
      title: 'Parameter Batch'
    },
    body: {
      page: 1,
      limit: 10,
      keyword: '',
      year: '',
      active: '',
      target: '',
      sort: 'latest'
    },
    paging: {},
    state: {
      isLoading: false,
      dialogDelete: false,
      selectedId: null
    },
    items: [],
    headers: [
      {
        text: 'No',
        value: 'no',
        sortable: false,
        class: 'dark--text h7--xxsmall dark lighten-2'
      },
      {
        text: 'Nama Batch',
        value: 'batch.title',
        sortable: false,
        class: 'dark--text h7--xxsmall dark lighten-2'
      },
      {
        text: 'Aksi',
        value: 'action',
        align: 'center',
        sortable: false,
        class: 'dark--text h7--xxsmall dark lighten-2'
      }
    ]
  }),

  created() {
    let appBar = {
      title: 'Parameter Batch',
      child: false
    }

    this.$store.commit('SET_META', appBar)
    this.setQuery(this.queryParams)
    this.fetch()
  },

  methods: {
    setQuery(val) {
      this.body.page = Number(val?.page) || this.body.page
      this.body.limit = Number(val?.limit) || this.body.limit
      this.body.keyword = val?.keyword || ''
      this.body.year = val?.year || this.body.year
    },
    async fetch() {
      this.state.isLoading = true

      const res = await this.$api.assessment.parameter.getList(this.body)

      if (res.success) {
        this.items = res.data.list?.map((x, index) => ({
          no: (this.body.page - 1) * this.body.limit + index + 1,
          ...x
        }))
        this.paging = res.data.paging
        this.paging['totalPage'] = Math.ceil(
          this.paging?.count / this.body.limit
        )
      }

      if (!res.success) {
        this.setFailedAlert(res)
      }

      this.state.isLoading = false
    },

    onSearch: debounce(function (val) {
      this.body.keyword = val || ''
      this.body.page = 1
      this.fetch()
      this.pushQuery('keyword', val)
    }, 500),

    onFilterYear(val) {
      this.body.year = val?.key !== 'semua' ? val?.key : ''
      this.fetch()
      this.pushQuery('year', val?.key)
    },

    async onDelete() {
      this.state.isLoading = true
      const id = this.state.selectedId

      const res = await this.$api.assessment.parameter.deleteParameter(id)

      if (res.success) {
        this.state.dialogDelete = false
        this.setSuccessAlert('Berhasil menghapus data')
        this.fetch()
      }

      if (!res.success) {
        this.setFailedAlert(res)
      }

      this.state.isLoading = false
    },

    handleDialogDelete(id) {
      if (id) {
        this.state.selectedId = id
      }

      this.state.dialogDelete = !this.state.dialogDelete
    },

    onChangePagination(val) {
      this.body.page = val
      this.fetch()
    },

    onDetail(id) {
      this.$router.push({
        path: `/admin/assessment/batch/${id}`
      })
    }
  }
}
</script>
