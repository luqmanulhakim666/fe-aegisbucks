<template>
  <div>
    <general-table-header
      ...
      actionCreate="/admin/assessment/parameters/create"
      filterYear
      @on:search="onSearch"
      @on:filterYear="onFilterYear"
    />
    <v-data-table
      :headers="headers"
...
export default {
  ...
  methods: {
    setQuery(val) {
      this.body.skip = Number(val?.skip) || this.body.skip
      ...
      this.body.year = Number(val?.year) || this.body.year
    },
    onFilterYear(val) {
      this.body.year = val?.key !== 'semua' ? val?.key : ''

      this.body.page = 1
      this.fetch()

      this.pushQuery('year', val?.key !== 'semua' ? val?.key : '')
    },
    ...
    async onDuplicate(id) {
      ...
      const res = await this.$api.assessment.label.duplicate(id)
    ...
    onEdit(id) {
      this.$router.push(`/admin/assessment/parameters/${id}`)
    },

    onDetail(id) {
      this.$router.push(`/admin/assessment/parameters/${id}?detail=true`)
    }
  }
}
</script>
