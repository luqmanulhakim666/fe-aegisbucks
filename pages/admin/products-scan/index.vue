...
export default {
  ...
  methods: {
    ...
    async onUpdateStatus(val) {
      const res = await this.$api.products.scan.update(val.id, {
    ...
    async fetch() {
      ...
      const res = await this.$api.products.scan.getList({ ...this.body });
    ...
    async onDelete() {
      ...
      const res = await this.$api.products.scan.delete(id);
    ...
    onEdit(id) {
      this.$router.push(`/admin/products-scan/${id}`);
    },
  },
};
</script>
