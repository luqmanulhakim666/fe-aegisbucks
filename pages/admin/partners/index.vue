...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ...
      const res = await this.$api.partners.getList({ ...this.body });
    ...
    async onDelete() {
      ...
      const res = await this.$api.partners.delete(id);
    ...
    onEdit(id) {
      this.$router.push(`/admin/partners/${id}`);
    },

    onExport() {
      const token = this.$store.getters["auth/isToken"];

      const url = this.$api.reports.export("retail", this.body, token);
      window.open(url);
    },
  },
};
