    ...
    <template v-if="!state.isLoading">
      ...
      <div class="white pa-6 rounded-xl">
        <v-form v-model="state.isValid" ref="form">
          <form-brands v-if="!state.isLoading" :form="form" />
        </v-form>
      </div>
      <!-- END FORM -->
...
export default {
  ...
  created() {
    this.meta.title = this.isCreated ? "Create New Brand" : "Edit Brand";
  ...
  methods: {
    async getOne() {
      ...
      const res = await this.$api.brands.getOne(id);
    ...
    async onSubmit() {
      ...
      if (valid) {
        ...
        let res = this.isCreated
          ? await this.$api.brands.create(payload)
          : await this.$api.brands.update(this.form.id, payload);
