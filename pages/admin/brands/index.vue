<template>
  <div>
    <general-table-header
      label="Brand"
      actionCreate="/admin/brands/create"
      @on:sort="onSort"
      @on:search="onSearch"
      @on:export="onExport"
      sort
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ...
      const res = await this.$api.brands.getList({ ...this.body });
    ...
    async onDelete() {
      ...
      const res = await this.$api.brands.delete(id);
    ...
    onEdit(id) {
      this.$router.push(`/admin/brands/${id}`);
    },

    onExport() {
      const token = this.$store.getters["auth/isToken"];

      const url = this.$api.reports.export("brand", this.body, token);
      window.open(url);
    },
  },
};
